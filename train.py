...
parser.add_argument("--algo", type=str, help="algorithm: [REINFORCE, TRPO, PPO]", default="PPO")
parser.add_argument("--seeds", type=int, nargs="+", default=10) # edit: originally list, now one seed at a time.
...
parser.add_argument("--hidden_dim", type=int, help="number of nodes in hidden layer", default=512)
parser.add_argument("--p_lr", type=float, help="policy learning rate", default=0.003)
parser.add_argument("--v_lr", type=float, help="value function learning rate", default=0.0002)
parser.add_argument("--n_epochs", type=int, help="PPO # epochs", default=5)
parser.add_argument("--eps_clip", type=float, help="PPO eps clip", default=0.2)
parser.add_argument("--entropy_coeff", type=float, help="PPO entropy coeff", default=0.005)
...
if __name__ == "__main__":
	...
	# edit: code will output only one seed
	# too long! one seed will take ~1 hour.
	# uncomment below and indent to use if you want multiple seeds.

	# for seed in args.seeds:
	# create seed dir
	seed_dir = os.path.join(algo_path, str(args.seeds)) # args.seeds was seed
	if not os.path.exists(seed_dir):
		os.makedirs(seed_dir)

	# train policy with the current seed
	train(args.seeds, seed_dir)
